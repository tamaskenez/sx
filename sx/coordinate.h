...
namespace details {
    ...
    class coordinate_facade : public std::array<ValueType, Rank> {
        static_assert(std::is_fundamental<ValueType>::value,
            "ValueType must be fundamental!");
    ...
    protected:
        ...
        {
            return *(const super_type*)this == (const super_type&)rhs;
        }
        ...
        friend _CONSTEXPR ConcreteType operator*(value_type v,
    ...
    public:
        ...
        const T operator*() const _NOEXCEPT { return val; }
...
public:
    ...
    {
        static_assert(rank == 1,
            "Single-parameter ctor enabled only when rank == 1");
    }
...
public:
    ...
    // Preconditions: component_idx < rank
    _CONSTEXPR const_reference
    operator[](size_type component_idx) const _NOEXCEPT
    {
        assert(component_idx == 0);
        (void)component_idx;
        return value;
    ...
    _CONSTEXPR index operator+() const _NOEXCEPT { return *this; }

    _CONSTEXPR index operator-() const _NOEXCEPT { return index(-value); }
...
public:
    ...
    friend _CONSTEXPR bounds operator+(const index<rank>& lhs,
        const bounds& rhs)
    {
        return rhs + lhs;
    }
    ...
    {
        for (int i = 0; i < rank; ++i) {
            if (idx[i] < 0 || idx[i] >= (*this)[i])
                return false;
        }
        return true;
    }

    const_iterator begin() const _NOEXCEPT { return const_iterator{ *this }; }
...
template <int Rank>
struct bounds_iterator
    : public std::iterator<std::random_access_iterator_tag, index<Rank>,
          ptrdiff_t, const details::arrow_proxy<index<Rank> >,
          const index<Rank> > {
    typedef std::iterator<std::random_access_iterator_tag, index<Rank>,
        ptrdiff_t, const details::arrow_proxy<index<Rank> >,
    ...
    explicit bounds_iterator(bounds<Rank> bnd,
        index<Rank> curr = index<Rank>{}) _NOEXCEPT
        : bnd(std::move(bnd)),
          curr(std::move(curr))
    {
    }

    reference operator*() const _NOEXCEPT { return curr; }
...
struct bounds_iterator<1>
    : public std::iterator<std::random_access_iterator_tag, index<1>, ptrdiff_t,
          const details::arrow_proxy<index<1> >, const index<1> > {
    // Preconditions: bnd.contains(curr) unless bnd.size() == 0
    ...
    reference operator[](difference_type n) const _NOEXCEPT { return curr + n; }
    ...
    void swap(bounds_iterator& rhs) _NOEXCEPT { std::swap(curr, rhs.curr); }
...
operator+(typename bounds_iterator<Rank>::difference_type n,
    const bounds_iterator<Rank>& rhs) _NOEXCEPT
{
    return rhs + n;
}
