...
public:
    ...
    explicit range_view_iterator(T value) : value_(value) {
    }
...
#define SX_DEF(OP) bool operator OP (const range_view_iterator& y) const { return value_ OP y.value_; }
    SX_DEF(<)
    SX_DEF(>)
    SX_DEF(<=)
    SX_DEF(>=)
...
public:
    ...
    explicit range_view(T to) : from_(0), to_(to) {
        assert(0 <= to);
    }

    range_view(T from, T to) : from_(from), to_(to) {
        assert(from <= to);
    }
